...
그런데 **`상위 수준 모델`에 대한 이해 없이 각각의 `객체 단위`만 바라보게 된다면 어떨까? 🤔**   
        
전반적인 구조나 큰 수준에서 도메인간의 파악이 어려워지므로 코드의 변경이나 확장을 기피하게 될 것이며      
**도메인 전문가와 대화를 나누고 코드를 분석해야** 비로소 상위 수준에서 모델간의 관계가 이해될 것이며 
더불어,**도메인 객체 모델이 복잡하다면 이러한 문제는 더욱 심각해진다.**        
        
이러한 문제를 해결하기 위해서 `객체 모델`에 **애그리거트를 적용해야한다.**          
