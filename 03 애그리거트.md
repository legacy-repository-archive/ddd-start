...
`상위 수준 모델`을 통해 `개별 객체 단위`를 나타내면 위와 같다.          
         
그런데 **`상위 수준 모델`에 대한 이해 없이 각각의 `객체 단위`만 바라보게 된다면? 🤔**                
도메인 전문가와 대화를 나누고 코드를 분석해야 비로소 상위 수준에서 모델간의 관계가 이해되기 시작한다.     
     
더불어,**도메인 객체 모델이 복잡해지면 개별 구성요소 위주로 모델을 이해하게 되고       
