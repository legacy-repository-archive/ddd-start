...
```java
    ...
    public void cancel(OrderNo orderNo) {
        ...
        try {
            refundService.refund(getPaymentId());
            this.refundStatus = State.REFUND_COMPLETED; <-- 외부 서비스 성능에 직접 영향을 받는다.   
        } catch (Excetpion) {
            ... // 생략 
        }
    }
...
```java
```
예를 들어, 위와 같이 타입 파라미터로 PasswordChangedEvent를 사용한   
EventHandler 임의 클래스 구현이 있다고 하자       
...
자바 8 이전 버전이라면 아래와 같이   
EventHandler 인터페이스와 이 인터페이스를 구현한 추상 클래스를 구현하면 된다.   
