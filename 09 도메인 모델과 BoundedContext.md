...
이렇게 하나의 하나의 BOUNDED CONTEXT에서 여러 하위 도메인을 개발할 때는 주의점이 있는데       
바로, 하위 도메인의 모델들이 뒤섞이지 않게 해야 하는 것이다.              
...
따라서 한 개의 BOUNDED CONTEXT에서      
여러 하위 도메인을 포함하더라도 하위 도메인마다 구분되는 패키지를 갖도로 구현해야 한다.        
