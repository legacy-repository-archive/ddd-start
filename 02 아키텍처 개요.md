...
```
엔티티는 변경이 필요하면 엔티티를 새로 생성하지 않고,       
기존 엔티티를 조회해서 엔티티를 수정하는 방법을 사용하시는 것이 맞다.   
```
사실 엔티티는 도메인 로직을 가지고 있지만, 자료구조를 

엔티티는 변경이 필요할 경우, 불변을 유지할 이유가 없습니다.       
변경이라는 행위를 해도 식별자로 구별할 수 있기 때문입니다.     
    
실제 제가 속한 다른 스터디에서도      
`엔티티면 모든 필드가 변할 수 있도록 해줘야하지 않나요?`라는 말을 하기도..     
답은 없으니 한 번 생각해보면 좋을 것 같습니다.   
